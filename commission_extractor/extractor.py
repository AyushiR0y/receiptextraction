...
_agent_cache_lock = threading.RLock()  # RLock because backfill_agent_pan calls itself indirectly
...
# Patterns that identify the RECIPIENT (BALIC/Bajaj) — never the broker.
_BALIC_NAME_RE = re.compile(
    r"bajaj\s*allianz|balic|bajaj\s*finance|bajaj\s*finserv|bajaj\s*life|"
    r"bajaj\s*allianz\s*life",
    re.IGNORECASE,
)


def _is_balic_name(name: str) -> bool:
    """Return True if *name* looks like a Bajaj/BALIC entity rather than the broker."""
    return bool(_BALIC_NAME_RE.search(name or ""))


def _agent_name_from_filename(source_file: str) -> str:
    """Try to derive the broker name from the PDF filename.

    File names are typically structured as '<BrokerName> <date range>.pdf'.
    Strip trailing date/month/year tokens and normalise.
    """
    stem = Path(source_file).stem
    # Remove leading underscore or punctuation
    stem = re.sub(r"^[_\-\s]+", "", stem)
    # Drop trailing date-like tokens: e.g. "apr 26", "1-15 may26", "2025", "Apr-May25"
    stem = re.sub(
        r"[\s_\-]*(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z\s\-_0-9]*$",
        "",
        stem,
        flags=re.IGNORECASE,
    )
    stem = re.sub(r"[\s_\-]*(fy\s*20\d\d[\-_]?\d*|20\d\d[\-_]\d+|[0-9]{1,2}[\-\/][0-9]{1,2}[\-\/]?\d*)[\s\d]*$", "", stem, flags=re.IGNORECASE)
    stem = re.sub(r"[\s_\-]*\d+[\-\s]?\d*\s*$", "", stem)
    return stem.strip()
...
def backfill_agent_pan(fields: Dict[str, str], source_text: str, source_file: str = "") -> Dict[str, str]:
    result = dict(fields)

    # --- Guard: if AI returned a Bajaj/BALIC entity as the agent, clear it so we can recover ---
    raw_name = (result.get("Agent Name", "") or "").strip()
    if _is_balic_name(raw_name):
        LOGGER.debug("Clearing Bajaj/BALIC entity '%s' from Agent Name field", raw_name)
        result["Agent Name"] = ""

    # --- If Agent Name is still missing, try to infer from filename ---
    if not (result.get("Agent Name", "") or "").strip() and source_file:
        inferred = _agent_name_from_filename(source_file)
        if inferred and not _is_balic_name(inferred):
            # Only adopt if it fuzzy-matches a known agent (or if no agent info loaded yet)
            if AGENT_CODE_BY_NAME:
                code_guess, clean_guess, sim = find_best_matching_agent_code(inferred)
                if sim >= 0.55 and code_guess:
                    result["Agent Name"] = clean_guess or inferred
                    if not (result.get("AGENT_CODE", "") or "").strip():
                        result["AGENT_CODE"] = code_guess
                    LOGGER.debug(
                        "Agent name recovered from filename '%s' -> '%s' (sim=%.2f, code=%s)",
                        source_file, result["Agent Name"], sim, code_guess,
                    )
            else:
                result["Agent Name"] = inferred
    ...
    # --- If we have a name but no code, try fuzzy code lookup ---
    if name and not code and AGENT_CODE_BY_NAME:
        code_guess, clean_guess, sim = find_best_matching_agent_code(name)
        if sim >= 0.65 and code_guess:
            result["AGENT_CODE"] = code_guess
            if not name or sim >= 0.85:
                result["Agent Name"] = clean_guess or name
            code = code_guess
            LOGGER.debug("Fuzzy-matched agent code %s for name '%s' (sim=%.2f)", code_guess, name, sim)
    ...
    if pan_ok:
        ...
        if info:
            ...
            if not (result.get("Agent Name", "") or "").strip() and info.get("name"):
                result["Agent Name"] = info.get("name")
...
def _load_agent_details_sheet(df: "pd.DataFrame") -> None:
    """Populate AGENT_INFO_BY_PAN / AGENT_INFO_BY_CODE from any DataFrame that has
    AGENT_CODE, Name, and PAN columns (exact names or common aliases)."""
    for _, row in df.iterrows():
        ...
        gstn = str(row.get("GSTN", "") or row.get("Agent GSTN", "") or row.get("Broker GSTN", "") or "").strip().upper()
        ...
        if code:
            ...
            # seed name lookup so backfill also works by name
            if name:
                nk = _normalize_agent_name_key(name)
                AGENT_CODE_BY_NAME.setdefault(nk, code)
                AGENT_NAME_BY_CODE.setdefault(code, name)
        if name and pan:
...
from .tax_logic import apply_confident_math_fill, apply_gst_autofill
...
def extract_receipts_with_azure_llm(page_text: str, source_file: str, page_num: int, context_hint: str = "") -> List[Dict[str, str]]:
    ...
    # Build a short hint list of known agent names (up to 60) so the AI can pick the right one
    _known_agent_hint = ""
    if AGENT_NAME_BY_CODE:
        sample_names = list(AGENT_NAME_BY_CODE.values())[:60]
        _known_agent_hint = (
            " Known broker/agent names (for reference only — do NOT hallucinate codes): "
            + "; ".join(sample_names[:60])
            + ". "
        )

    system_prompt = (
        "You are extracting commission invoice data from OCR text of receipts issued BY insurance brokers/agents TO Bajaj Allianz Life Insurance (BALIC). "
        "Return JSON only with top-level key 'receipts'. If no receipt is present return {\"receipts\": []}. "
        "Never invent or guess values — if a field cannot be found in the text, leave it blank.\n\n"

        "=== CRITICAL AGENT vs RECIPIENT RULE ===\n"
        "These invoices are RAISED BY the broker/agent and SENT TO BALIC/Bajaj Allianz. Therefore:\n"
        "  • 'Agent Name'  = the BROKER / INTERMEDIARY / IMF who ISSUED this invoice (the SENDER). "
        "    This is never 'Bajaj Allianz', 'BALIC', 'Bajaj Finance', or any Bajaj entity. "
        "    Look for the company name at the top of the invoice, under 'Bill From', 'Issued By', or similar. "
        "    It often appears alongside a GSTN, PAN, or agent code on the letterhead.\n"
        "  • 'Name of Service Receipient' = BALIC / Bajaj Allianz Life Insurance Co. Ltd. (the RECEIVER).\n"
        "  • 'Agent PAN' = PAN of the broker who issued the invoice (10-char format AAAAA9999A). "
        "    NOT the PAN of any Bajaj entity.\n"
        "  • 'BROKER GSTN' = GSTIN of the broker/agent (starts with state code, 15 chars). "
        "    NOT the BALIC/Bajaj GSTIN.\n"
        "  • 'BALIC GSTN'  = GSTIN belonging to Bajaj Allianz / BALIC (the recipient).\n\n"
        + _known_agent_hint

        + "Use these exact field keys: "
        "Agent Name, Agent PAN, Name of Service Receipient, BALIC STATE, BALIC GSTN, "
        "BROKER GSTN STATE, BROKER GSTN, Vendor Inv Date, Vendor Inv No, Total Inv Amt, "
        "BROKERAGE Amount, CGST @ 9%, SGST @ 9%, UTGST, IGST, GST TOTAL AMT, Narration.\n\n"

        "Field rules:\n"
        "1) Vendor Inv No: the invoice number only — not a GSTIN, PAN, HSN/SAC, date, page number, or total.\n"
        "2) Vendor Inv Date: a date (DD/MM/YYYY or similar). Leave blank if absent.\n"
        "3) Total Inv Amt: grand total of the invoice. BROKERAGE Amount: taxable brokerage before GST. "
        "GST TOTAL AMT: only the GST/tax portion.\n"
        "4) Tax mode: use IGST alone OR CGST+SGST/UTGST alone — never both. Map only non-zero tax fields; "
        "zero fields stay blank. GST TOTAL AMT must equal the non-zero tax mode sum.\n"
        "5) Narration: short label like COMMISSION. Blank if absent.\n"
        "6) Multiple receipts on one page → one JSON object per receipt.\n"
        "7) Never copy the invoice number from the filename."
    )

    if bajaj_finance_context:
        system_prompt += (
            "\n8) Bajaj Finance page: return only the single summary row whose Particulars cell says 'Total'. "
            "Ignore all intermediate line items. If no Total row is visible, return {\"receipts\": []}."
        )

    user_prompt = (
        f"Source file: {source_file}\n"
        f"Page: {page_num}\n"
        "Extract all receipts from the text below. Only include values explicitly visible in the text. "
        "If a value is ambiguous, leave it blank.\n"
        + (f"Structured GST context: {context_hint}\n" if context_hint else "")
        + "Text:\n"
        + page_text[:24000]
    )
...
# AFTER — exist_ok=True already makes mkdir safe, but the write can
# still race on the same cache key from two threads on the first run.
# Simplest fix: write to a temp file then rename (atomic on Linux).
def _ocr_cache_put(cache_key: str, text: str) -> None:
    if not _ocr_cache_enabled():
        return
    try:
        OCR_CACHE_DIR.mkdir(parents=True, exist_ok=True)
        cache_file = OCR_CACHE_DIR / f"{cache_key}.txt"
        # Atomic write: write to .tmp then rename so a partial write
        # from another thread is never read as a valid cache entry.
        tmp_file = cache_file.with_suffix(".tmp")
        tmp_file.write_text(text or "", encoding="utf-8")
...
def get_rapidocr_engine():
    global RAPIDOCR_ENGINE, RAPIDOCR_INIT_FAILED
    if RapidOCR is None or RAPIDOCR_INIT_FAILED:
        return None
    if RAPIDOCR_ENGINE is not None:
        return RAPIDOCR_ENGINE
    with _rapidocr_lock:
        if RAPIDOCR_ENGINE is None and not RAPIDOCR_INIT_FAILED:
            try:
...
def get_easyocr_reader():
    ...
    if easyocr is None or EASYOCR_INIT_FAILED:
        return None
    if EASYOCR_READER is not None:
        return EASYOCR_READER
    with _easyocr_lock:
        if EASYOCR_READER is None and not EASYOCR_INIT_FAILED:
            try:
...
def pdf_page_texts(pdf_bytes: bytes, source_name: str, override_password: Optional[str]) -> List[Tuple[int, str]]:
    ...
    if reader.is_encrypted:
        ...
        opened = False
        # Re-create PdfReader per attempt to avoid state carry-over between decrypt attempts
        for candidate in passwords_to_try:
            try:
                try:
                    temp_reader = PdfReader(io.BytesIO(pdf_bytes))
                except Exception:
                    # If we cannot create a fresh reader, skip this candidate
                    LOGGER.debug("Failed to instantiate PdfReader for %s", source_name, exc_info=True)
                    continue
                # attempt decrypt on fresh reader
                try:
                    result = temp_reader.decrypt(candidate)
                except Exception:
                    result = None
                # Interpret pypdf return codes robustly: int(result) != 0 indicates success in modern pypdf
                success = False
                try:
                    if result is None:
                        success = False
                    ...
                    success = bool(result)
                LOGGER.debug("Trying password candidate '%s' for %s -> decrypt result: %r (success=%s)", candidate, source_name, result, success)
                if success:
                    opened = True
                    reader = temp_reader
                    LOGGER.info("Decrypted protected PDF %s using candidate '%s' (raw=%r)", source_name, candidate, result)
                    break
            except Exception:
                # keep trying other candidates
                continue
        if not opened:
            # pypdf failed to decrypt; try PyMuPDF (fitz) as a fallback
            try:
                if fitz is not None:
                    for candidate in passwords_to_try:
                        try:
                            doc = fitz.open(stream=pdf_bytes, filetype="pdf")
                            if doc.is_encrypted:
                                # fitz.authenticate expects an empty string for no-password
                                ok = doc.authenticate(candidate or "")
                            else:
                                ok = True
                            if ok:
                                LOGGER.info("Decrypted protected PDF %s using PyMuPDF candidate '%s'", source_name, candidate)
                                page_texts = []
                                for i in range(doc.page_count):
                                    text = doc.load_page(i).get_text() or ""
                                    page_texts.append((i + 1, normalize_text(text)))
                                doc.close()
                                return page_texts
                        except Exception:
                            continue
            except Exception:
                # if fitz isn't available or it fails, continue to next fallback
                pass

            # Next fallback: pikepdf (qpdf). If available, use it to create a decrypted temp file and read with pypdf
            try:
                import tempfile
                import os
                import shutil
                    ...
                    for candidate in passwords_to_try:
                        try:
                            ...
                            with open(tmp_path, "wb") as fh:
                                fh.write(pdf_bytes)
                            # try opening with pikepdf using candidate
                            try:
                                with pikepdf.open(tmp_path, password=candidate or "") as pdf:
                                    # save decrypted to another temp file
                                    dec_tmp_fd, dec_tmp_path = tempfile.mkstemp(suffix=".pdf")
                                    os.close(dec_tmp_fd)
                                    pdf.save(dec_tmp_path)
                                    # read decrypted with pypdf
                                    with open(dec_tmp_path, "rb") as dec_fh:
                                        dec_reader = PdfReader(dec_fh)
                                        page_texts = []
                                        for i, page in enumerate(dec_reader.pages, start=1):
                                            raw_text = page.extract_text() or ""
                                            page_texts.append((i, normalize_text(raw_text)))
                                        # cleanup
                                        try:
                                        ...
                                        try:
                                            ...
                                            pass
                                    LOGGER.info("Decrypted protected PDF %s using pikepdf candidate '%s'", source_name, candidate)
                                    return page_texts
                            ...
                            # cleanup and continue if outer try fails
                            try:
                                os.remove(tmp_path)
                            except Exception:
                                pass
                            continue
                # if pikepdf not available or none succeeded, raise
            except Exception:
                pass
    ...
    # Fallback: if many pages are empty/too-short from pypdf, try PyMuPDF text extraction
    try:
        low_quality_pages = [p for p, t in page_texts if len((t or "").strip()) < 40]
        if low_quality_pages and fitz is not None:
            try:
                doc = fitz.open(stream=pdf_bytes, filetype="pdf")
                for p in low_quality_pages:
                    try:
                        fz_page = doc.load_page(p - 1)
                        # use 'text' layout to get reasonable plain text
                        fz_text = normalize_text(fz_page.get_text("text") or "")
                        # Replace only when fitz yields clearly better text
                        if len(fz_text.strip()) > len((page_texts[p - 1][1] or "").strip()):
                            page_texts[p - 1] = (p, fz_text)
                    except Exception as exc:
                        LOGGER.debug("PyMuPDF per-page text extraction failed for %s page %s: %s", source_name, p, exc)
                doc.close()
            except Exception as exc:
                LOGGER.debug("PyMuPDF fallback failed to open %s: %s", source_name, exc)
    except Exception:
        # Don't let fallback break normal flow
        pass
...
def _is_bajaj_finance_total_row(receipt_text: str, fields: Dict[str, str]) -> bool:
    ...
    if not _is_bajaj_finance_context(receipt_text) and not _is_bajaj_finance_row(fields):
        return False
    particulars_total = bool(re.search(r"\btotal\b", row_text, re.IGNORECASE))
    row_has_amount = any(
        bool(clean_amount(fields.get(k, "")))
        for k in ["Total Inv Amt", "BROKERAGE Amount", "GST TOTAL AMT", "CGST @ 9%", "SGST @ 9%", "IGST"]
    )
    ...
    for page_num, page_text in page_texts:
        ...
        if ai_first:
            ...
            for ai_row in ai_rows_first:
                ...
                ai_row = backfill_agent_pan(ai_row, page_text, source_file=effective_display)
...
def process_image(path: Path, source_hint: Optional[Path] = None, source_display: Optional[str] = None) -> List[ReceiptLineItem]:
    ...
    for page_num, text in extract_text_from_image_file(path):
        ...
        fields = backfill_agent_pan(fields, text, source_file=effective_display)
        if (fields.get("Narration", "") or "").strip().upper() == "COMMISSION":
            inferred_narr = infer_narration_from_source_name(effective_display)
            if inferred_narr:
                fields["Narration"] = inferred_narr
...
def apply_agent_code_mapping_to_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    For each row:
    - Backfill Agent Name / AGENT_CODE / Agent PAN from the loaded agent details
    - If AGENT_CODE is still empty, try to find it via agent name similarity matching
    ...
    df = df.copy()
    identity_columns = [col for col in ("Agent Name", "AGENT_CODE", "Agent PAN", "BROKER GSTN") if col in df.columns]
    
    for idx, row in df.iterrows():
        row_fields = {}
        for column in identity_columns:
            value = row[column]
            row_fields[column] = (str(value) if pd.notna(value) else "").strip()
        updated_fields = backfill_agent_pan(row_fields, "")
        for column in identity_columns:
            updated_value = (updated_fields.get(column, "") or "").strip()
            if updated_value and updated_value != row_fields.get(column, ""):
                df.at[idx, column] = updated_value

        agent_name = (updated_fields.get("Agent Name", "") or "").strip()
        agent_code = (updated_fields.get("AGENT_CODE", "") or "").strip()
        ...
        if not agent_code and agent_name and agent_name != "nan":
            ...
            if matched_code:
                ...
                if "Agent PAN" in df.columns and matched_code in AGENT_INFO_BY_CODE:
                    matched_info = AGENT_INFO_BY_CODE[matched_code]
                    if not (str(df.at[idx, "Agent PAN"]) if pd.notna(df.at[idx, "Agent PAN"]) else "").strip() and matched_info.get("pan"):
                        df.at[idx, "Agent PAN"] = matched_info.get("pan")
                    if "BROKER GSTN" in df.columns and not (str(df.at[idx, "BROKER GSTN"]) if pd.notna(df.at[idx, "BROKER GSTN"]) else "").strip() and matched_info.get("gstn"):
                        df.at[idx, "BROKER GSTN"] = matched_info.get("gstn")
                LOGGER.debug("Final mapping: '%s' -> code %s (similarity: %.2f)", agent_name, matched_code, similarity)
    
    return df

# AFTER — drop-in replacement with threading + memory optimisations
def _process_one_file(
    file_path: Path,
    password: Optional[str],
) -> tuple[str, List[ReceiptLineItem]]:
    """
    Process a single file and return (source_str, rows).
    Designed to run inside a ThreadPoolExecutor worker.
    Errors are caught here so one bad file never kills the batch.
    """
    source_str = str(file_path)
    try:
        rows = process_path(file_path, password)
        if not rows:
            rows = [build_placeholder_row(
                source_str, "", "No identifiable receipt data extracted from document"
            )]
        LOGGER.info("Done: %s → %d row(s)", file_path.name, len(rows))
        return source_str, rows
    except Exception as exc:
        LOGGER.exception("Failed to process %s: %s", file_path, exc)
        return source_str, [build_placeholder_row(
            source_str, "", f"Extraction failed: {exc}"
        )]
...
def run(
    ...
    """
    Process all files under input_path in parallel then write a single
    output Excel file.

    max_workers=8 on an 8-vCPU / 16 GB pod is the sweet spot when OCR
    is via Google Vision (network-bound, not CPU-bound).  Each thread
    spends most of its time waiting for the Vision API response, so all
    8 threads stay busy without saturating the CPU.

    Tune down to 4 if you hit Google Vision 429 rate-limit errors.
    """
    ...
    candidate_files = list(find_candidate_files(input_path))
    total = len(candidate_files)
    LOGGER.info("Starting parallel run: %d file(s), %d worker(s)", total, max_workers)

    all_rows: List[ReceiptLineItem] = []
    processed_sources: List[str] = []
    completed = 0

    # ThreadPoolExecutor: GIL is released during urllib network I/O (Vision API calls)
    # and during PyMuPDF/PIL rendering (C extensions), so threads run truly in parallel
    # for the bottleneck operations.
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(_process_one_file, fp, password): fp
            for fp in candidate_files
        }
        for future in concurrent.futures.as_completed(futures):
            source_str, rows = future.result()
            processed_sources.append(source_str)
            all_rows.extend(rows)
            completed += 1
            # Progress every 10 files so you can see it moving in logs
            if completed % 10 == 0 or completed == total:
                LOGGER.info("Progress: %d/%d files complete", completed, total)

    # --- Memory optimisation: build DataFrame in one shot from the
    #     complete list rather than concatenating incrementally, then
    #     immediately drop the raw list so GC can reclaim it. ----------
    df = rows_to_dataframe(all_rows)
    del all_rows  # free ~50-100 MB of dicts before the xlsx write
    ...
    LOGGER.info(
        "Usage | vision_calls=%s | azure_calls=%s | azure_in_chars=%s | azure_out_chars=%s",
